...
type SQLScriptHandler struct{ DB *sql.DB }
...
func (h *SQLScriptHandler) Execute(ctx context.Context, tool *DynamicTool, params map[string]any) (string, error) {
	...
	for i, sc := range stmtsConfig {
		...
		args := resolveTemplateParams(stmtParams, params)
...
func resolveTemplateParams(paramsConfig []any, params map[string]any) []any {
	var args []any
	for _, p := range paramsConfig {
		tmpl, ok := p.(string)
		if !ok {
